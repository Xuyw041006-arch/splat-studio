...
try:
    ...
    if not masks24.exists():
        ...
        command([sys.executable,'-u',SCRIPT,'prepare-extra',*COMMON,'--training-masks',MASKS,'--extra-views','16','--strict-query-labels','--output',OUT/'masks24'],'prepare-extra.log')
    cases=[('aligned-improved-24','balanced','improved',masks24),('aligned-improved-24-fine','fine','improved',masks24)]
    ...
    plan={'cases':[{'name':n,'geometry':g,'preset':p,'masks':str(m)} for n,g,p,m in cases],
          ...
          'gt_read_after_all_training':True,'protocol':str(ROOT/'docs/SEMANTIC_V2_PROTOCOL_R2.zh-CN.md')}
